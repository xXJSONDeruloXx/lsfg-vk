...
LsContext::LsContext(const Hooks::DeviceInfo& info, VkSwapchainKHR swapchain,
    ...
    std::vector<int> outFds(Config::calculateMaxGenerationCount(conf.multiplier));
    for (size_t i = 0; i < Config::calculateMaxGenerationCount(conf.multiplier); ++i)
    ...
    lsfgInitialize(
        ...
        conf.hdr, 1.0F / conf.flowScale, Config::calculateMaxGenerationCount(conf.multiplier),
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        const auto maxGenerationCount = Config::calculateMaxGenerationCount(conf.multiplier);
        pass.renderSemaphores.resize(maxGenerationCount);
        pass.acquireSemaphores.resize(maxGenerationCount);
        pass.postCopyBufs.resize(maxGenerationCount);
        pass.postCopySemaphores.resize(maxGenerationCount);
        pass.prevPostCopySemaphores.resize(maxGenerationCount);
    }
}
...
VkResult LsContext::present(const Hooks::DeviceInfo& info, const void* pNext, VkQueue queue,
    ...
    const auto currentGenerationCount = Config::calculateVariableGenerationCount(conf.multiplier, this->frameIdx);
    std::vector<int> renderSemaphoreFds(currentGenerationCount);
    for (size_t i = 0; i < currentGenerationCount; ++i)
    ...
    VkSemaphore lastPrevPostCopySemaphore =
        pass.prevPostCopySemaphores.at(currentGenerationCount - 1).handle();
    const VkPresentInfoKHR presentInfo{
        .sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR,
