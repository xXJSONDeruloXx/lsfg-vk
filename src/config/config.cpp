...
void Config::updateConfig(const std::string& file) {
    ...
    if (global.multiplier < 2.0)
        throw std::runtime_error("Global Multiplier cannot be less than 2.0");
    ...
    for (const auto& gameTable : gamesList.as_array()) {
        ...
        Configuration game{
            .enable = true,
            .dll = global.dll,
            .multiplier = toml::find_or(gameTable, "multiplier", 2.0),
...
Configuration Config::getConfig(const std::pair<std::string, std::string>& name) {
    ...
    if (std::getenv("LSFG_LEGACY")) {
        ...
        if (multiplier) conf.multiplier = std::stod(multiplier);
